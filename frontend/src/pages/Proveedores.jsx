...
export default function Proveedores() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editing) {
        await axios.put(`${API}/api/proveedores/${editing.id}`, form)
      } else {
  ...
  const handleDelete = async (row) => {
    if (!confirm(`¿Desactivar el proveedor "${row.nombre}"?`)) return
    try {
      await axios.delete(`${API}/api/proveedores/${row.id}`)
      fetchData()
    } catch (err) {
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">{data.length} proveedores registrados</p>
        </div>
        <button
          onClick={openCreate}
